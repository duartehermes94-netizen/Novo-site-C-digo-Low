...
// Replace the string manually
const searchStr = `function FaqItem({ question, answer, isOpen = false }: { question: string, answer?: string, isOpen?: boolean }) {  return (    <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden transition-all group">      <div className="p-5 flex items-center justify-between cursor-pointer">        <div className="flex items-center gap-3">          <div className={\`w-6 h-6 rounded-full flex items-center justify-center shrink-0 \${isOpen ? 'bg-gray-200' : 'bg-gray-100 group-hover:bg-gray-200'} transition-colors\`}>            <Plus className={\`w-4 h-4 text-gray-500 \${isOpen ? 'rotate-45' : ''} transition-transform\`} />          </div>          <h4 className="font-semibold text-brand-dark text-lg">{question}</h4>        </div>      </div>      {isOpen && answer && (        <div className="px-5 pb-5 pt-1 pl-14 text-gray-600 font-medium">          {answer}        </div>      )}    </div>  );}`;
...
faqCode = faqCode.replace(searchStr, newFaqItem);
